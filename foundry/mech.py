...
CHOC_V2_STAB_SOURCES = {
    ...
    # length = 上面図の全長 18.75（ねじの側の丸い端から爪の輪 φ3.50 の端まで）・ring = 爪の輪の径（下面図 φ3.50）。
    # プレートの高さで奥にいちばん出る所 = 8.50 + ring/2 = 10.25、手前 = length − 10.25 = 8.50（CHOC_V2_STAB_PLATE_BACK）
    "drawing": dict(pivot=12.0, box=(6.00, 8.00), screw=(-6.20, 3.00, 3.00), claw=(8.50, 4.00, 4.00),
                    part=(5.80, 7.30), length=18.75, ring=3.50,
                    # 高さ（基板の上面から。側面図）: 軸の上端 8.60・箱の上面 5.00・ねじのボスの上面 2.85。
                    # 軸の十字 3.95±0.05 × 1.26±0.03（上面図）・ワイヤ φ1.20（下面図）
                    stem_top=8.60, box_top=5.00, boss_top=2.85, slider=(3.95, 1.26), wire=1.20),
...
#   スイッチの開口と羽の間の桟（x 6.975〜8.87・y 6.64375 より下）だけが残る
# 変えた所（羽の外の辺と奥の端のほかは、どれも穴を広げる向き）:
...
#   - 桟の上の小さな突起（x 7.55〜8.375・y 6.64〜7.14）を穴に含めた。STAB_KERF で広げると幅 0.525 の
#     突起になり、刷れない（foundry.plate.thin_webs が見つけた・2026-09-26）
#   - **羽の奥の端 10.85 → 10.72（CHOC_V2_STAB_PLATE_BACK）**。10.85 に STAB_KERF を足すと、1 段奥のスイッチの
#     開口（キーの中心から 19.05 − 13.95/2 = 12.075）との間の帯が 1.075 で、PLATE_MIN_WEB 1.2 を割っていた
#     （スタビのキー 4 つの羽 8 つ全部）。10.72 なら帯 1.205。スタビがプレートの高さ（基板の上 1.0〜2.2）で
#     いちばん奥に出る所は、図（CHOC_V2_STAB_SOURCES の drawing）の寸法で爪の輪 8.50 + φ3.50/2 = **10.25**
#     （全長 18.75 = ねじの側の端 8.50 + 10.25 と合う）。10.72 + STAB_KERF 0.15 との間は 0.62。
#     その先の 0.70（上面図・10.95 まで）は基板の穴 φ4.0 を通って基板の下で開く爪の先と読んだ（側面図で爪の返しは
#     基板の下）。**読みなので見本で確かめる**（決定記録 2026-09-25-choc-v2 §10-6）
...
CHOC_V2_STAB_PLATE_BACK = 10.72
CHOC_V2_STAB_PLATE = ((0.0, 6.64375), (8.87, 6.64375), (8.87, -9.45), (15.0375, -9.45),
                      (15.0375, CHOC_V2_STAB_PLATE_BACK), (8.375, CHOC_V2_STAB_PLATE_BACK), (8.375, 9.85),
                      (0.0, 9.85))
...
def choc_v2_stab_plate_polys(at=(0.0, 0.0), outline=None, web=0.0, kerf=0.0):
    ...
    羽を外形の外まで伸ばすと、左右の羽のあいだ（スイッチの開口の手前の板と、開口と羽の間の桟）が
    外形の側で切り離される。**その部分は捨てずに「枠」として別に刷る**（foundry.plate.plate_frames）。
    スイッチの ±x の辺の爪とつばが枠を挟む。前は枠ごと抜いて、スペースの 2 キーのスイッチの周りに
    板が無かった（2026-09-25。tests/test_cckb.py の test_every_switch_is_framed_by_plate が見る）
    """
    ax, ay = at
