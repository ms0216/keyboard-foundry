...
#      = この系列のどの軸にも共用できるようにこの形で開ける）。どちらも中心から (5.15, 5.00)。
#      **底面図の「5.50」は穴の位置ではなく、穴の中に来る下面の小さな突起**（細い矩形 約 0.4 × 0.9 の中心。
#      その手前に約 0.5 × 0.4 の突起が約 4.71。2026-09-26 に静音 茶の図を 600dpi で描き出し、5.15 の寸法線が
#      破線の長円の中心、5.50 が矩形の中心に当たることを画素で読んだ。標準の底面図にも同じ突起がある）。
#      丸い足ではない。**穴は 2 枚の推奨の穴の和を含む丸 φ2.1**: 和（中心から 1.0）を JLC の丸穴の公差
#      −0.08 でも含む（2.1 − 0.08 = 2.02）。突起のいちばん遠い角は穴の中心から約 0.79 で、φ2.02 から位置の
#      公差 ±0.05 を引いても 0.17 余る（tests/test_choc_v2.py が見る）。
#      **前は長円 1.6 × 2.0 で開けていた**が、JLC の「長円の長さは幅の 2 倍以上」を満たさず（比 1.25）、非めっきの
#      長円の公差 ±0.2 で幅 1.4 になりうる（2026-09-26 の監査 C 重要 1・B 軽微 2）。Kailh の静音の推奨の長円 2.0 × 1.5 も
#      比 1.33 で同じ規則を満たさない。比 2 の長円（例 1.6 × 3.2）は、奥へ伸ばすと行のバスに近づき、幅は公差で 1.2 まで縮む
#      → lib/keyswitch.pretty/SW_Kailh_Choc_V2_Common（kiswitch の SW_Kailh_Choc_V2 から位置決め穴だけ替えた物）
#   2. つばの下面から爪の先までの隙（プレートが入る所）: 標準 1.65・静音 **1.35**（下の plate_t）
CHOC_V2_FP = "SW_Kailh_Choc_V2_Common"
# 位置決めの穴の中に来る下面の突起（静音 茶の底面図を画素で読んだ。[暫定]・見本で確かめる）。
# KiCad の足跡の座標（中心から。Y 下向き）で (x0, y0, x1, y1)
CHOC_V2_LOCATOR_NUBS = ((-5.45, 5.30, -4.55, 5.70),     # 細い矩形 約 0.9 × 0.4（図の 5.50 の寸法が指す物）
                        (-5.445, 4.50, -4.935, 4.92))   # 小さな角 約 0.5 × 0.4（中心から約 4.71）

# Choc V2 用のねじ留めスタビ（遊舎工房 A050001-01-1「Choc v2用スタビライザー 2U スクリュータイプ」）。
# 座標は**支点を原点・X は外向き（キーの中心から遠ざかる向き）・Y はワイヤ（爪）の側が +**（CAD。ワイヤは
# 常に奥に置くので +Y = 奥）。出典は 2 つで、どちらもメーカーの公式ではない。**正は salicylic**（2026-09-26 利用者と決めた）:
#   salicylic  販売者（サリチル酸さん）の足跡 kbd_Stabilizer.pretty/Choc_v2_PCBMountStab_2u（commit 9ade20b）。
#              販売者いわく「実際のパーツの実測から割り出しています」。**支点 ±11.9（23.8）**
#   drawing    販売者が仕入れ先から取って公開した図（記事 chocv2-stab-guide の「Recommended PCB Layout・
#              PCB-H=1.6mm」。2026-05-16。支点の間 24.00）。販売者いわく「参考程度」。**参照だけ**
#              （足跡に無い値——箱の大きさ・高さ・十字・爪の輪——はここから読む。[暫定]・見本で測る）
# 前（2026-09-25）は 2 つの和で穴を開けていた（長円 3.2 × 3.4・4.2 × 4.4）。和は「どちらでも入る」だけを見て、
# **ゆるさの害**（締める前にスタビが ±0.2 動く・監査 E 重要 3）と JLC の長円の比（C 重要 1）を見ていなかった
# （docs/method/lessons.md K）。
CHOC_V2_STAB_SOURCES = {
    "salicylic": dict(pivot=11.9, box=(6.0, 7.5), screw=(-6.2, 3.2, 3.4), claw=(8.24, 3.0, 4.0)),
    # part = 箱そのもの（図の上面図 5.80 × 7.30）
...
# 基板に開ける穴（支点 CHOC_V2.stab_offset = 11.9 から。X は外向き）。**2026-09-26 に利用者と決めた値**:
#   box    箱（基板の下へ 3.30 出る）の穴。Edge.Cuts で抜く（interface.stab_reliefs）。(x0, y0, x1, y1)。
#          x は支点 ± 3.0（足跡・図とも 6.0）。y は ±4.0（図の推奨 8.00。足跡の 7.5 より広い）。
#          箱（図の 5.80 × 7.30）との隙は x 片側 0.1・y 片側 0.35。**外形の公差 ±0.2 で x がきつければ、やすりで広げる**
#          （箱の穴の縁から 0.35 は外形の配線禁止の帯。ねじの穴との橋は 6.2 − 1.5 − 4.0 = 0.7）
#   screw  ねじの穴。**丸** φ3.0（非めっき）を y −6.2。((中心 x, 中心 y), 径)。ナットの胴 φ2.78 が下から入る
#          （片側 0.11。JLC の丸穴の公差 −0.08 でも 2.92）。足跡の長円 3.2 × 3.4 は JLC の長円の比を満たさない
#   claw   ワイヤの側の爪の穴。**丸** φ4.0 を y +8.24（足跡の位置。図は 8.50）
# どちらの穴も**きつい側**（発注後に削って広げられる）。穴の中心から STAB_HOLE_KEEPOUT_R（spec）の中には
# GND のベタのほかに銅を置かない（手でリーマ・やすりをかけても信号に当たらない。pcb_extra の STAB_HOLE_KEEPOUT）。
# tests/test_choc_v2.py が足跡の値と、tests/test_cckb_pcb.py が発注する板の穴と突き合わせる
CHOC_V2_STAB_PIVOT = 11.9
CHOC_V2_STAB_HOLES = dict(box=(-3.0, -4.0, 3.0, 4.0),
                          screw=((0.0, -6.2), 3.0),
                          claw=((0.0, 8.24), 4.0))
CHOC_V2_STAB_FP = "Stab_Kailh_Choc_V2_Screw_2u"
# プレートの開口（片側・**キーの中心を原点**・X は外向き・+Y = ワイヤ = 奥）。軸に平行な辺だけ。
# 出典: サリチル酸さんの kbd_SW_Hole.pretty/Stab_Hole_Choc_v2_PCBMount_2u（commit 9ade20b。支点 11.90625。
# 寸法の画像では支点の間 23.8087）の値のまま:
#   羽（スタビ本体が通る）x 8.875〜14.9375（幅 6.0625）・y −9.45（半円の頂）〜10.85
#   ワイヤの帯: **スイッチの開口の上辺からそのまま y 9.85 まで**（|x| < 8.375）。開口・帯・羽が 1 つの U 字の穴で、
#   スイッチの開口と羽の間の桟（x 7.05〜8.875 = 1.82）だけが残る
# **足跡から変えたのは次の 5 つだけ**（どれも理由があり、決定記録 2026-09-25-choc-v2 §10-9 にも書く）:
#   1. **羽の奥の端 10.85 → 10.72（CHOC_V2_STAB_PLATE_BACK）**: 10.85 に STAB_KERF を足すと、1 段奥のスイッチの
#      開口（キーの中心から 19.05 − 13.95/2 = 12.075）との帯が 1.075 で、刷れる帯の下限 PLATE_MIN_WEB 1.2 を割る。
#      10.72 なら帯 1.205。スタビがプレートの高さ（基板の上 1.0〜2.2）でいちばん奥に出る所は、図（参照）の寸法で
#      爪の輪 8.50 + φ3.50/2 = **10.25**。その先の 0.70（上面図・10.95 まで）は基板の穴を通って基板の下で開く爪の先と
#      読んだ。**読みなので見本で確かめる**（§10-5 V9）
#   2. スペースの羽の手前（外形から 0.225 しか残らない）を外形の外まで伸ばし、切り離された枠を別に刷る
#      （choc_v2_stab_plate_polys・foundry.plate.plate_frames。O15 で利用者の判断待ち。変えない）
#   3. 桟の上の小さな突起（x 7.55〜8.375・y 6.64〜7.14）を穴に含めた（STAB_KERF で広げると幅 0.525 になり刷れない。
#      foundry.plate.thin_webs が見つけた・2026-09-26）
#   4. 半円・角の丸みは外接する矩形にした（軸に平行な辺だけにする。**穴が大きくなる向きだけ**）。羽の内の辺は
#      左右で 8.87125 / 8.875 と 0.004 違うので、左右の和（8.87125）にした（これも穴が大きくなる向き）
#   5. （機種の置き方で起きる）CCKB の左 Shift の左の羽は、角の開口（XIAO の角）へ 0.075 かかってプレートの外の開口と
#      つながる。角へ板を伸ばす所が無い。スイッチの枠は右の羽の外側（幅 1.58）で板につながる（§10-6。2 回目の V2 監査 E-7）
# 前（2026-09-25〜26）は支点 12.0（図）でも入るよう羽の外の辺を +0.1（15.0375）広げていた。支点を足跡の 11.9 に
# したので戻した。刷った PLA の穴の縮みは STAB_KERF（HHKB #30 の実測）が受け持つ（形の妥協ではない）
# プレートでは STAB_KERF だけ外へ広げて開ける（plate.build_plate）
CHOC_V2_STAB_PLATE_BACK = 10.72
CHOC_V2_STAB_PLATE = ((0.0, 6.64375), (8.87125, 6.64375), (8.87125, -9.45), (14.9375, -9.45),
                      (14.9375, CHOC_V2_STAB_PLATE_BACK), (8.375, CHOC_V2_STAB_PLATE_BACK), (8.375, 9.85),
                      (0.0, 9.85))
...
def choc_v2_stab_plate_polys(at=(0.0, 0.0), outline=None, web=0.0, kerf=0.0):
    ...
    スイッチの ±x の辺の爪とつばが枠を挟む。**枠はプレートにも基板にも留まらず、スイッチに挟まって
    ぶら下がるだけ**（スイッチを留めるのは基板のはんだ。枠は見た目と爪の掛かる所。2026-09-26 の監査 E 重要 4）。
    前は枠ごと抜いて、スペースの 2 キーのスイッチの周りに板が無かった（2026-09-25。tests/test_cckb.py の
    test_every_switch_is_framed_by_plate が「周りに板があるか」を見る。**板がスイッチを保持するかは見ない**）
    """
    ax, ay = at
...
CHOC_V2 = Switch(
    ...
    # （支点 11.9 − 3.0 = x 8.9。2026-09-25 は 12.0 − 3.1 − 0.3 = 8.6）とコートヤード（x ±1.15 → 8.25）の間に
    # 0.65 残す（V1 はキー 4 つだけ DIODE_OVERRIDE で動かしていた。全部のキーで同じ位置にして上書きを無くした）。
    # 端子 2 (5, −3.8) からアノード（x 7.1）へ L 字 2 本。アノードのパッドと端子 2 のランドの間 0.55
    diode_offset=(7.1, -1.0),
    diode_angle=90,
    stab_offset={2.0: CHOC_V2_STAB_PIVOT, 2.25: CHOC_V2_STAB_PIVOT},
    stab_fp={CHOC_V2_STAB_PIVOT: CHOC_V2_STAB_FP},
