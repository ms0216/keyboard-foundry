...
- 利用者の買い物に **MK-12C02-G025（LCSC C778186）** を足した（1 台 1 個。予備を含めて 3 個を勧める）。**同日に秋月の SS-12D00G3 へ置き換え**（[2026-09-24-power-switch.md](2026-09-24-power-switch.md)）
...
- 指の窪み（当時の定数。2026-09-24 の電源スイッチの置き換えで窪みごと無くなった）を 1.0 → **0.9**: 壁 1.3 − 1.0 では窪みの奥の壁が 0.3（線 1 本に満たない）。0.9 なら 0.4、
...
- **母数**: 取付の穴は板に 11（H0〜H9・H_LID）。`interface.metal_on_pcb()` が穴ごとに金属（ナット・インサート・
  ...
  | H0〜H9 | 下からの皿ネジの頭（z 0.9〜2.1。床 1.2 の後は 0.5〜1.7・基板の下面 3.0） | 当たらない（基板の下面 3.4 との間はトレイの樹脂のボス） | — |
...
**← 2026-09-25 に置き換えた**: task-10a はダイオードを使わず、可変電源で XIAO の起動に要る電圧を測り、ダイオードは
Vf の保証値（BAT46W ≦0.45V@10mA）で紙の上に載せる。O12 は同日 BAT46W で決まった（open-gaps O12）。
...
XIAO の裏の USB のシールドのめっきの長穴 4 個を `spec.XIAO_BOTTOM_SLOTS` として数え、検査で名指し（D4）。**〔訂正・3 回目の監査 軽微 2〕「禁止域の形は変わらない」は誤り**: XIAO_UNDERSIDE は 3 → 5 個になり、USB の端の長穴 2 個＋0.3 の矩形（x 6.819〜9.198）が XIAO の端 6.855 から 0.036 はみ出す（面積 +0.107mm²）。はみ出しは基板の縁と EDGE_KEEPOUT の中で、表の銅は変わらない（3 回目の監査が塗った後のベタで確かめた）。
...
- R2: JLC の公式の資料が「はみ出す SMT 部品はキャリア」「Economic は縁から 0.3」と書いていること。
  ...
  **→ 電源スイッチの置き換え（decisions/2026-09-24-power-switch.md）で読み直し済み**: いまの SS-12D00G3 はスルーホールで
  LCSC に無い（秋月で買う）ので、JLC の実装に戻す道はもう無い
