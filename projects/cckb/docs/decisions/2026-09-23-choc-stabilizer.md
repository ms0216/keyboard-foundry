...
| 3 | 開口とハウジングの隙間 | Keebio の輪郭は奥行 +0.25・切り欠き +0.4、**幅は 0**（6.30 = ハウジング 6.30 +0.02/−0.05）。プレートでは輪郭を片側 0.15（`STAB_KERF`）広げて開ける | 外：Keebio の輪郭値と製造図のハウジング寸法の差分。広げる量は HHKB #30 の刷った PLA での実測（Cherry）。**Choc では未実測** |
| 4 | ワイヤの向き | 常に奥（切り欠きが奥側） | 外：製造図とKeebioの輪郭の対応（Chocでは最前列でも回さない） |
| 5 | スイッチ開口とスタビ開口の間の桟 | ≧1.6mm（0.4×4以上）。広げたあとで 1.80mm | 自分の生成物（CHOC_STAB_OUTLINE − STAB_KERF と cutout=13.8 の差分計算）。刷れる幅として検査で担保 |
...
- `plate.choc_stab_polygons(s, at)` で左右 2 つの開口の多角形を返し、`build_plate` はこれを
  `stab_cutout_face(s, polygon=...)`（Cherry と同じ面の作り方）で面にしてプレートから引く。
  **開口は Cherry と同じく `STAB_KERF`（片側 0.15）だけ外へ広げる**（`stab_cutout_face(..., polygon=...)`
  の多角形オフセット・`Kind.INTERSECTION`）。Keebio の輪郭は幅がハウジングと同じ 6.30 で隙間が
  0 だった。刷った PLA の穴は締まるので、このままでははまらないおそれがある（最終レビューで判明）。
  `CHOC_STAB_OUTLINE` 自体は出典の値のまま変えない。Choc での嵌め合いは**未実測**で、
  試し刷りの小片ではめて押して確かめる（open-gaps）。
...
| 項目 | 決められない理由 |
|---|---|
| 開口の嵌め合い（片側 0.15 広げた値でよいか） | 0.15 は Cherry スタビを刷った PLA で測った値。Choc のハウジングでは測っていない。試し刷りの小片ではめて押して確かめる |
