...
- **電源スイッチの入の向きは図の読み**（SS-12D00G3。レバーを奥へ押すと入。open-gaps P5）。**利用者が手はんだ**（付ける前にテスターで確かめられる）。置き換えの記録は `decisions/2026-09-24-power-switch.md`。配線は **`route_pcb.py --reroute VBAT_IN,VBAT_SW --clear 95.25,-47.925,143.175,-28.575`**（右の角の 2 網だけ）
- **USB を挿す前に電源スイッチを OFF**（ショットキー D_PWR の逆漏れ。BAT46W〔O12 で決定〕では 25℃ で電池へ入らないが高温は保証が無い。open-gaps 受け入れた差 A1・O12）
- ~~電源スイッチの位置決めの穴がホルダの下に来る DRC（npth_inside_courtyard 2 件）は警告に下げた~~
  → 2026-09-24 に電源スイッチを替えて穴が無くなったので、下げるのをやめた（spec.DRC_SEVERITY を消した。検査が .kicad_pro の error を見る）
