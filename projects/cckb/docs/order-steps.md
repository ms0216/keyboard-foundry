...
**先に LCSC と遊舎工房に部品を頼む**（[shopping-list.md](shopping-list.md) §1・§3）。下の 1〜3 は届いた部品で確かめる。
...
| 3 | **電源スイッチの入の向き**（open-gaps **P5**） | 届いた MK-12C02-G025 のつまみを片側へ寄せ、テスターの導通で ②③（真ん中と、端子が 1.5 間隔の側）が繋がる向きを見る。設計は「つまみを ③ の側（手前）へ寄せると入」と図面から読んだ | Claude に言う（`circuit.py` の ①③ を入れ替える。発注前なら直せる） | 図面の読みのまま頼まれる。違っていたら「手前が切」になるだけで壊れない（説明書を逆にする） |
...
ケースの形（トレイ＋ふた・O8）も**基板の取付穴に乗っている**ので、上枠の案に戻したいならこの時点で言う
（[status.md](status.md) の「代わりに決めたこと」）。継ぎ目（O9）・厚さ（O10）はケースだけで、基板の後でよい。
...
| 目 | やること | 対象 |
|---|---|---|
| B 商品写真 | LCSC・スイッチサイエンスの商品写真で 1 番（丸・帯・＋の印）の位置を見て、KiCad の板の絵（`build/pcb_view/…__U_MCU.png`・`__BT1.png`・`__SW_PWR.png`）と向きを合わせる | 74LVC595・XIAO・MK-12C02-G025・BS-16-B4AK003 |
| C KiCad でクリック | KiCad で `cckb_main.kicad_pcb` を開き、パッドをクリックしてネット名を読む: U1 の 12 番＝CS、XIAO の D7＝CS、BT1 の＋（パッド 1）＝VBAT_IN、SW_PWR の ②＝VBAT_IN・③＝VBAT_SW | 同上 |
| D 1:1 の印刷 | KiCad から板を 1:1 で紙に印刷（「実際のサイズ」）し、届いた実物（XIAO・電池ホルダ・電源スイッチ）を載せて、足とパッドが重なり＋の向きが合うかを見る | XIAO・BS-16-B4AK003・MK-12C02-G025 |
