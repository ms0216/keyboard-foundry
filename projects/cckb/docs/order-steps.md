...
**結論: 基板のデータは出来ている。頼む前に利用者がやることが 6 つ残っている（下の §1。2 の O12 は 2026-09-25 に決まった）。
...
**先に日本橋の店頭（電源スイッチ）・遊舎工房（スタビ）で部品を買う**（[shopping-list.md](shopping-list.md) §2・§3）。下の 3・4 は届いた部品で確かめる。1 は部品を待たずにできる（手持ちの物だけ）。

| 順 | 何（台帳の番号） | どうやって閉じるか | 駄目だったら | 決めないとどうなるか（既定） |
|---|---|---|---|---|
| 1 | **CR1632 で起動できるか**（open-gaps **#1**・門を止めている） | [task-10a-coin-cell-startup.md](task-10a-coin-cell-startup.md) を上から。可変電源から抵抗 0/25/50/75/100Ω を挟んで XIAO の 3V3 へ（ダイオードなし）、電圧を下げながら各段で入れ直し、5 回とも起動する最低の電圧を測る。新品の CR1632 で 1 回突き合わせる。電池とダイオード（BAT46W の保証値）は紙の上で載せる。合格の基準は手順書 §6 の (a) 21℃ の新品・(b) −10℃ の新品。結果の「使える量」で open-gaps **O14**（バルクコンデンサの場所を置くか。96mAh 未満なら決める）も決まる。所要 約 1.5 時間・買う物なし | CR2032 に替える判断を Claude に返す（ホルダ・右の角の設計をやり直す。基板も変わる） | 門が開かない（発注できない） |
| 2 | ~~**D_PWR の品番**（open-gaps **O12**）~~ **2026-09-25 に決まった: BAT46W（C54110）** | 何もしない。保証値で決まった（Vf ≦0.45V@10mA・25℃ では USB 給電中に電源 ON でも電池へ逆流しない）。比べた表は [decisions/2026-09-24-audit-fixes.md](decisions/2026-09-24-audit-fixes.md) §S2 | — | BAT46W で頼まれる。1 の判定はこの Vf の保証値で見る |
...
| 5 | **アンテナのまわりを ¥0 で 1 回測る**（open-gaps **P7**・強く勧める） | [docs/knowledge/rf-antenna.md](../../../docs/knowledge/rf-antenna.md) の手順。XIAO の手前と奥をアルミ箔で覆い、M2 ネジ 1 本とインサート 1 個を H3 の位置に置いて（4 回目の監査 D 軽微 1）、スマホの nRF Connect で RSSI を覆う前と比べる | 20dB 以上悪くなったら Claude に言う（基板の改版） | 試作で RSSI を見るまで分からない |
...
| Tooling holes | JLC に任せる（Added by JLCPCB） | Economic では位置決めの穴（φ1.152 の NPTH を対角の隅に 2〜3 個）が必須で、この板には無いので**JLC が必ず足す**（JLC の記事 How to add tooling holes）。左上・右上・右下の隅には空きがあるが、**左下（XIAO の角）は 20×20 の範囲に空きが無い**（4 回目の監査 C 軽微 1）→ 下の Confirm Production file で位置を見る |
| **Confirm Production file** | **Yes** | JLC が返す製造データで、足した位置決めの穴の位置を §3 で見るため |
...
**見積もりに「Keyboard Design」の追加料金（$16.41＋$49.25/m²。5 枚なら約 $23）が乗ることがある**。JLC の追加料金の規定
（<https://jlcpcb.com/help/article/in-what-cases-will-there-be-charged-extra>。判定の基準は書かれていない）で、スタビの逃げ穴 8 個と
キーのスイッチ 62 個からキーボードと判定される見込みが高い。規定どおりの料金なので断る理由にならない（4 回目の監査 C 軽微 2）。
参考に、部品代と Extended の段取り費だけで 2 枚実装 約 $10.8（[shopping-list.md](shopping-list.md) §6）。
...
**位置決めの穴**（Confirm Production file で JLC が返すデータ）: JLC が足した φ1.152 の穴が、線・部品・XIAO の下に
掛かっていないかを見る。**左下（XIAO の角）に来ていたら払わずに断り、別の隅を頼む**（空きがあるのは左上・右上・右下。
4 回目の監査 C 軽微 1。GND のベタに穴が 1 つ開くだけなら害は無い）。
...
| 目 | やること | 対象 |
|---|---|---|
| B 商品写真 | LCSC・Amazon（XIAO）・共立エレショップ（電源スイッチ・ホルダ）の商品写真で 1 番（丸・帯・＋の印）の位置を見て、KiCad の板の絵（`build/pcb_view/…__U_MCU.png`・`__BT1.png`・`__SW_PWR.png`）と向きを合わせる | 74LVC595・XIAO・SS-12D00G3・BS-16-B4AK003 |
