...
**結論: 基板のデータは出来ている。頼む前に利用者がやることが 5 つ残っている（下の §1。2 の O12 は 2026-09-25 に決まり、4 の見本は 2026-09-26 に基板の後へ回した）。
全部済んだら、ZIP・BOM・CPL の 3 ファイルを JLCPCB に上げ、下の表のとおりに選び、配置のプレビューで
部品の向きを 4 か所見てから払う。**

- 発注の門: `tools/kb cckb gate` がいま「**閉 — #1・#4**」を出す（2026-09-26 に #6〔V2 の見本〕を取り下げて再び確かめた）。#1 と #4 を閉じると「開」になる
...
**先に日本橋の店頭（電源スイッチ）で部品を買う**（[shopping-list.md](shopping-list.md)）。下の 3 は届いた部品で確かめる。4（スイッチ・スタビの見本）は**基板を頼んだ後でよい**（2026-09-26）。1 は部品を待たずにできる（手持ちの物だけ）。
...
| 4 | ~~**スイッチ・スタビの見本で基板に効く所**（open-gaps **#6**）~~ **2026-09-26 取り下げた（基板の後でよい）** | スタビの穴は販売者の足跡（実物の実測）、スイッチの穴は Kailh の推奨の足跡で決まっている（外の事実）。穴はきつい側に開けてあり、入らなければ手で広げる（手順書 手順 5）。見本は刷る物（キャップの受け口・小片）を合わせるときに測る（open-gaps **P13**。前の (a) 下面の突起が φ2.1 に入るか・(b) 支点・箱・ナットの胴・爪の輪・ねじの頭はそこへ移した） | 支点・箱・胴・爪が穴より大きくて手で広げきれないとき（0.3 超）は Claude に言う | 足跡（販売者の実測）の値で頼まれる |
...
| キーのダイオード（**62 個とも縦**。板の上で 90°。スタビのキー D42・D43・D58・D60 も同じ向き。例 D1） | 帯（カソード＝パッド 1・行の網）が**手前（−y）** | 帯が手前の側 | `build/pcb_view/cckb.pcb.cckb_main__SW54.png` |
| D_PWR（電源のショットキー。板の上で 180°） | 帯（カソード＝パッド 1・V3V3）が**右**（XIAO の 3V3 へ行く側） | 帯が右の側（プレビューは左右が裏返るので、画面では左に見えることがある——表から見た向きに直して判定する） | `build/pcb_view/cckb.pcb.cckb_main__D_PWR.png` |
...
**JLC の DFM が「穴と基板の縁（hole to board edge）」を出したとき**: スタビのねじの穴 φ3.0 と箱の穴（外形の内側の穴・Edge.Cuts）の間の基板の橋は 0.7 しか無い（決定記録 2026-09-25-choc-v2 §10-9。わざと決めた値・手順書で締めすぎない）。JLC がこれを見つけて「穴を動かす・小さくする」などの直しを提案しても、**中身を読んでから**決め、黙って受け入れない（穴の位置と径はスタビの実測に合わせてある。動かすとスタビが付かない）。分からなければ払わずに Claude に言う。
