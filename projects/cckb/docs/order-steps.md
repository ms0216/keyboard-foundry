...
| 4 | **スイッチ・スタビの見本で基板に効く所**（open-gaps **#6**・**P13**。V2 = `cckb-v2` の枝） | 届いた Choc V2（静音）と遊舎工房のねじ留めスタビ A050001-01-1 を 1 組: (a) スイッチの下面の突起が位置決めの丸穴 φ2.1 に入る大きさか（ノギス）。(b) スタビの支点の間（両端の軸の中心の間）が 23.8 か・箱が 5.80 × 7.30 か・ナットの胴が φ2.78 か・爪の輪が φ3.5 か・ねじの頭（φ3.7〜3.85 × 1.5〜1.6 と読んだ）。ワイヤは奥（4 組とも）で、スタビはプレートに掛からない | 支点・箱・胴・爪が穴より**大きい**なら Claude に言う（穴はきつい側に開けてあり、0.3 までは手で広げられる。それより大きければ基板の変更）。ねじの頭が φ3.9 を超えればケースだけ直す（基板は変わらない） | 足跡（販売者の実測）の値で頼まれる |
